...
class DrugReminder extends React.Component {
  ...
  toUpdateStatusReminder({ index, _id, is_active }) {
    ...
    let currentItem = this.props.dataReminder.listReminder.data[index];
    currentItem = {
      ...currentItem,
      is_active: !is_active
    };
    this.props.updateItemListDrugReminder(index, currentItem);
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Gagal menghapus pengingat obat');
            }
          }
        }
      ],
  ...
  render() {
    const { data, isLoading } = this.props.dataReminder.listReminder;
    return (
...
const mapDispatchToProps = dispatch => ({
  ...
  updateItemListDrugReminder: (index, item) =>
    dispatch({
      type: 'UPDATE_REMINDER_LIST_ITEM',
      payload: {
        index,
        item
      }
